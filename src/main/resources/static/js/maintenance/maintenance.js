$(document).ready(function() {
    ...
    try {
        ...
        $('#maintenanceTable').DataTable({
            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, row, meta) {
                        try {
                            if (type === 'sort') {
                                return $(row).attr('data-order') || '';
                            }
                            return data;
                        } catch (e) {
                            console.error('Error in DataTables render:', e);
                            return '';
                        }
                    }
                }
            ...
            initComplete: function(settings, json) {
                ...
                $('#maintenanceTable_filter input').off('keyup').on('keyup', function(e) {
                    if (e.keyCode === 13) {
                        const searchValue = $(this).val();
                        window.location.href = '/maintenance?page=0&size=' + getPageSize() + '&search=' + encodeURIComponent(searchValue);
                    }
                });
            },
            error: function(error) {
                console.warn('DataTables error:', error);
                // Nonaktifkan DataTables jika error
                if ($.fn.DataTable.isDataTable('#maintenanceTable')) {
                    $('#maintenanceTable').DataTable().destroy();
                }
            }
    ...
    // Fungsi untuk mendapatkan ukuran halaman
    function getPageSize() {
        const urlParams = new URLSearchParams(window.location.search);
        return urlParams.get('size') || 10; // Default 10 jika tidak ada parameter
    };
